...
<head>
    ...
    <title>Admin - Point Assignment Alert</title>
    <link rel="preconnect" href="https://rsms.me/">
    <link rel="stylesheet" href="https://rsms.me/inter/inter.css">
</head>
<body class="bg-gradient-to-br from-blue-50 to-white text-gray-800 dark:from-gray-900 dark:to-gray-950 dark:text-gray-100">
    <div class="max-w-xl mx-auto my-8 overflow-hidden rounded-xl shadow-lg bg-white dark:bg-gray-800">
        
        <!-- Header -->
        <div class="px-6 py-4 bg-blue-600 text-white dark:bg-blue-700">
            <h2 class="text-xl font-bold">
                🔔 Point Assignment Alert - Admin Notification
            </h2>
        </div>
        ...
        <div class="px-6 py-6 text-sm leading-6">
            ...
            <p class="mb-4">
                A new point assignment has been made in the system:
            </p>
            ...
            <div class="mb-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
                ...
                <ul class="space-y-1 text-gray-700 dark:text-gray-300">
                    <li><span class="font-semibold">Points:</span> 
                        <span class="{{ $isPositive ? 'text-green-600 dark:text-green-400 font-bold' : 'text-red-600 dark:text-red-400 font-bold' }}">
                            {{ $assignment->points > 0 ? '+' : '' }}{{ $assignment->points }}
                        </span>
                    </li>
                    <li><span class="font-semibold">From:</span> {{ $assignor->name }} ({{ $assignor->email }})</li>
                    <li><span class="font-semibold">To:</span> {{ $recipient->name }} ({{ $recipient->email }})</li>
            ...
            @if($assignment->status === 'pending')
                <div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg dark:bg-yellow-900/20 dark:border-yellow-800">
                    <p class="text-yellow-800 dark:text-yellow-200 text-xs">
                        ⚠️ <strong>Action Required:</strong> This assignment is pending supervisor approval.
                    </p>
                </div>
            @endif
            ...
            <div class="mt-6">
                <a href="{{ config('app.url') }}/admin/dashboard" 
                   class="inline-flex items-center justify-center rounded-lg bg-blue-600 px-5 py-2 text-sm font-medium text-white transition hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800">
        ...
        <div class="px-6 py-4 text-center text-xs text-gray-500 bg-gray-50 dark:bg-gray-900 dark:text-gray-400">
            <p>This is an automated admin notification from the {{ config('app.name') }} Point Tracking System.</p>
            <p>Sent to hr@mnotify.com</p>
        </div>
    </div>
</body>
</html>